...
const validateEmployee = [
  body("firstname")
    ...
    .bail()
    .custom(nameAlpha)
    .withMessage("firstname must be alphabetic only"),
  body("lastname")
    .exists()
    ...
    .bail()
    .custom(nameAlpha)
    .withMessage("lastname must be alphabetic only"),
  body("dept")
    .exists()
    ...
    .bail()
    .custom(nameAlpha)
    .withMessage("dept must be alphabetic only"),
  body("title")
    .exists()
    .withMessage("title is required")
    .bail()
    .isIn(["Mr", "Miss", "Mrs", "Dr"])
    .withMessage("title must be one of Mr, Miss, Mrs, Dr"),
  body("birthDate")
    .exists()
    .withMessage("birthDate is required")
    .bail()
    .isISO8601()
    .withMessage("birthDate must be a date in ISO format (YYYY-MM-DD)")
    .bail()
    .custom((value) => {
      ...
      if (birth.isBefore(minDate))
        throw new Error("birthDate must be on or after 1900-01-01");
